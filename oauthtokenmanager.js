...
* is to always return a promise, even if we are using a locally cached access token. 
* We also normalise and errors to a fido2error.fido2Error.
*/
		...
		if (tokenResponse != null && tokenResponse.expires_at_ms > (now.getTime() + (2*60*1000))) {
			...
			fetch(
				...
				if (tr && tr.access_token) {
					...
					console.log("oauthtokenmanager fetch unexpected token response: " + (tr != null) ? JSON.stringify(tr) : "null");
